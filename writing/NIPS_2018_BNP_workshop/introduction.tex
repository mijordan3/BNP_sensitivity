...
% Additionally, we linearly approximate only the computationally
% intensive part of inference---the optimization of the global parameters---and
% retain the nonlinearity of easily computed quantities as functions of the global parameters.

% Additionally, we linearly approximate only the optimization
% of the posterior approximation for the global parameters; in VB, this
% step is typically the most computationally intensive part of
% inference. Meanwhile, we will show that we retain many other desirable
% non-linearities inherent in inference--including relations between
% global parameters, local parameters, and the number of distinct
% clusters
