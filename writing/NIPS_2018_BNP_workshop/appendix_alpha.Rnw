...
Equation~\ref{eq:expected_num_clusters_thresh} defines an \textit{in-sample} quantity, that is, the expected number of clusters we believe to be in the current Iris data set. We also consider a posterior predictive quantity, or the number of clusters we expect to see should we collect a \textit{new} data set of 150 iris flowers, given our posterior knowledge about the stick-breaking process. This is an expectation over the variational distribution of the sticks $\nu$, defined as 
\begin{align}
