...
source("./R_graphs/plot_sens_results.R")
...
results_pert1 <- filter(iris_data$results_df, !pred, pert=="1")
results_pert2 <- filter(iris_data$results_df, !pred, pert=="2")
results_pred_pert1 <- filter(iris_data$results_df, pred, pert=="1")
results_pred_pert2 <- filter(iris_data$results_df, pred, pert=="2")

grid.arrange(
    # First row --- include titles.
    plot_prior_perturbation(
            filter(iris_data$prior_pert_df,
                   filename == "prior_pert1.csv")) +
        theme(legend.position = c(0.85, 0.80)) +
        ggtitle("Priors"),
    plot_parametric_sensitivity(
            results_pert1, alpha_0 = -1, xlabel=TeX("$\\delta$")) +
        theme(legend.position = c(0.65, 0.81)) +
        ggtitle("In-sample quantity"),
    plot_parametric_sensitivity(
            results_pred_pert1, alpha_0 = -1, xlabel=TeX("$\\delta$")) +
        theme(legend.position =  "None") +
        ggtitle("Predictive quantity"),

    # Second row --- no titles.
    plot_prior_perturbation(
            filter(iris_data$prior_pert_df,
                   filename == "prior_pert2.csv")) +
        theme(legend.position = "None"),
    plot_parametric_sensitivity(
            results_pert2, alpha_0 = -1, xlabel=TeX("$\\delta$")) +
        theme(legend.position = c(0.65, 0.81)),
    plot_parametric_sensitivity(
            results_pred_pert2, alpha_0 = -1, xlabel=TeX("$\\delta$")) +
        theme(legend.position =  "None"),
