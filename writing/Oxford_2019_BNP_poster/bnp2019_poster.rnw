...
source("./R_graphs/iris/LoadData.R")
#source("./R_graphs/iris/LoadNumbers.R")
...
results_df_1 <- filter(!pred, pert=="alpha3")
results_df_2 <- filter(!pred, pert=="alpha8")
results_df_3 <- filter(!pred, pert=="alpha13")
grid.arrange(
...
results_pert1 <- filter(results_df, !pred, pert=="1")
results_pert2 <- filter(results_df, !pred, pert=="2")
results_pred_pert1 <- filter(results_df, pred, pert=="1")
results_pred_pert2 <- filter(results_df, pred, pert=="2")
...
grid.arrange(
    ...
    plot_prior_perturbation(
            filter(prior_pert_df, filename == "prior_pert2.csv")) +
        theme(legend.position = "None"),
    plot_parametric_sensitivity(
