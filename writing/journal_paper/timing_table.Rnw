...
given perturbation, as described in \secref{comp_amortization}.  Consistent with
this obervation, in all the examples, after the linear approximation is formed,
...
seven seconds, with subsequent refits (which recall were warm-started with the
