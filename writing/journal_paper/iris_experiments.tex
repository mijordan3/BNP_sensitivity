...
our Monte Carlo estimate of $\gclusterspred$ is a deterministic function of 
...
and we formed the linear approximation at this initial fit. 
...
the quantity $\gclusters(\etalinglobal(\alpha))$
varies only from 3.0 to
3.4
(recall that the true number of iris species is three). 
On the other hand, the posterior predictive quantity is sensitive 
to changes in $\alpha$.
Over the same range of $\alpha$, $\gclusterspred(\etalinglobal(\alpha))$
varies from 
3.6 to 
8.1. 
...
of our linearized variational (global) parameters. 
