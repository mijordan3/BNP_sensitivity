...
\begin{align*}
\g_{\text{n.cl}}(\eta) &= \expect{\q(\z\vert\eta)}{\sum_{k=1}^\kmax \ind{ \sum_{n=1}^{N}
\z_{\n\k} > 0}} \\ 
&= \sum_{k=1}^\kmax \left(1 -  \prod_{n=1}^N
\left(1 - \expect{\q(\z_{nk}\vert\eta)}{\z_{nk}}\right)\right).
\end{align*}
%
The in-sample quantity $\g_{\text{n.cl}}$ is an estimate for 
...
$g_{\text{n.cl.}}$ and $g_{\text{n.cl.pred}}$ 
...
\caption[The expected number of posterior predictive clusters as $\alpha$ varies in the 
the BNP-GMM fit of the iris data]{The expected number of posterior predictive clusters as $\alpha$ varies in the 
