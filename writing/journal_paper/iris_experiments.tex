...
On the left is the sensitivity of the in-sample quantity $\gclusters$.  
...
We compare against the expected number of clusters obtained by refitting the model at each $\alpha$ (blue). }\label{fig:iris_alpha_sens}
\end{figure}
...
When $\log\phi_\mu$ is centered at a location where the influence function is negative, the effect of the perturbation on $\gclusters$ 
...
In the last case, $\log\phi_\mu$ placed approximately equal weight on the negative and positive portions of the prior-weighted influence function, resulting in an approximately zero inner-product (\eqref{TODO}). 
...
\caption{Sensitivity of
        ...
        The prior-weighted influence function in purple, scaled to also have 
        unit $L_{\infty}$-norm. 
        ...
        as $\epsilon$ varies. }\label{fig:iris_fsens}
\end{figure}
