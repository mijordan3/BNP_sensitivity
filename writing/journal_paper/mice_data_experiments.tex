...
We include the additive offset becaues we 
are interested in clustering the pattern of gene expression, 
not the absolute level. 

The joint distribution is equivalent to~\eqref{bnp_model}, 
except that the conditional log-likelihood term includes $b_n$, 
in which case the first term in \eqref{bnp_model} becomes
$\logp(\x_n \vert \theta_\k, \b_n)$; 
and there is an additional prior term $\ell(\b_n)$. 
...
with parameters dependent on $k$. 
...
More precisely, let $\coclusteringmatr(\eta)$ denote the inferred co-clustering matrix; 
...
and we examine changes to this matrix,
$\Delta\coclusteringmatr(\eta) := 
\coclusteringmatr(\eta) - \coclusteringmatr_0$. 
We formed the linear approximation at $\alpha_0$ and computed
the change in the co-clustering matrix under the linearly approximated 
variational parameters, 
$\Delta\coclusteringmatr(\etalin(\alpha))$, at $\alpha = 1$ and $\alpha = 11$. 
Under the linear approximation, the changes in the co-clustering matrix 
is miniscue (\figref{gene_alpha_coclustering}), 
with the largest entries of the matrix on the order of $10^{-2}$. 
...
The linear approximation was also able to capture the sign and size of the changes in the individual entries of the coclustering matrix observed after refitting. 
...
Insensitivity to $\alpha$ does not necessarily rule out insensitivity to other prior perturbations. 
As demonstrated in \secref{results_iris},
the influence function provides guidance on which functional perturbations may result in greater sensitivity in a posterior quantity. 
However, the co-clustering matrix as a posterior quantity is an 
$\ngenes^2$-dimensional quantity and 
thus does not easily lend itself to an interpretable influence function. 
We therefore summarize the co-clustering matrix into a scalar quantity: 
we use the sum of the eigenvalues, or equvalently, 
the trace of the symmetrically normalized graph Laplacian. 
...
Because $\laplacianevsum(\eta)$ is a scalar, we can plot its influence function 
(\figref{gene_coclustering_influence}). 
We construct a functional perturbation $\log\phi$ that has a large inner-product 
with the influence function, thus having a large effect on $\laplacianevsum$. 
With $\laplacianevsum$ a proxy for our actual posterior quantity of interest, 
the full co-clustering matrix, we expect that the co-clustering matrix 
...
{\centering \includegraphics[width=0.980\linewidth,height=0.314\linewidth]{figure/gene_coclustering_influence-1} 

}

\caption[The influence function of $g_{ev}$, the sum of the eigenvalues of the coclustering Laplacian matrix]{The influence function of $g_{ev}$, the sum of the eigenvalues of the coclustering Laplacian matrix. }\label{fig:gene_coclustering_influence}
\end{figure}
...
We construct a $\log\phi$ using two Gaussian bumps aligned with
