...
$\etalinglobal(\alpha)$ for $\alpha = 1, \ldots, 7$. 
For this range of $\alpha$, $\etalinglobal(\alpha)$ and the refitted parameters 
...
Under the (globally) linearized variational parameters $\etalinglobal(\t)$, 
...
the linear approximation $\etalin(t)$ 
is a poor subsitute for the refitted parameters $\etaopt(t)$ 
(in this subsection, the only posterior quantities considered 
are functions of global parameters, so we do not make a distinction 
between $\etalin$ and $\etalinglobal$).  
Naturally, the linear approximation 
$\etalin(t)$ will be a poor substitute for $\etaopt(t)$ when 
the mapping $\t\mapsto\etaopt(\t)$. 
...
<<stru_fully_lin_example_cap>>=
  stru_fully_lin_example_cap <- paste(
    ...
    this approximation outperforms linearizing the variational parameters only. ")
