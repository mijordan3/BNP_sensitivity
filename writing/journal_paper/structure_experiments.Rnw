...
\Sexpr{sprintf('%.0f', weights_alpha_init$weight[1])} expected loci belonging to population1, 
\Sexpr{sprintf('%.0f', weights_alpha_init$weight[2])} belonging to population 2, and 
\Sexpr{sprintf('%.0f', weights_alpha_init$weight[3])} belonging to population 3. 
The remaining populations all have fewer than
\Sexpr{ceiling(weights_alpha_init$weight[4])} expected loci. 
Said another way, given a sample of population memberships, 
$\z\sim \q(\z\vert\eta)$, we would expect approximately
$\Sexpr{sprintf('%.0f', weights_alpha_init$weight[1])}$ of
the memberships to belong to population 1, 
$\Sexpr{sprintf('%.0f', weights_alpha_init$weight[2])}$ to belong to population 2, and so forth. 
...
Recall from \figref{stru_func_sens} that the linear approximation 
