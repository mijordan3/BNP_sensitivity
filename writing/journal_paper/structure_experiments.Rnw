...
individual admixtures the initital fit with
stick-breaking distribution is
$\pstick(\nu_{nk}) = \betadist{\nu_{nk}\vert 1, \alpha_0}$, $\alpha_0 = 6$. 
In this fit, there appears to be three dominant latent populations, 
...
with each having a loose correspondence with the Chawia, Ngangao, and Mbololo geographical regions.  
...
The expected number of in-sample clusters was 
...
(We also often use ``population" instead of ``cluster", in this application). 
...
Finally, the computational cost of linearizing the variational parameters is again favorable compared with the cose of refitting (\tabref{stru_timing}). 
...
for unconvering which perturbations might result in greater sensitiviy than others.
