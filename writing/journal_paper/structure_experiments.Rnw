...
Individuals were collected from four regions in southeast Kenya (Chawia, Mbololo, Ngangao, Yale), and each individual was genotyped at seven micro-satellite loci. 
...
Both implementations used a finite mixture model with a 
...
We still refer to $\gamma = (\nu, \latentpop)$ as global latent variables, even though the dimension of $\nu$ 
scales with the number of individuals, $\nindiv$. 
We call $\nu$ ``global" in this model because it does not scale 
with $\nindiv \times \nloci$ as $\z$ does. 
As usual, the optimal parameters $\eta_\z$ be solved in closed form as a function of the parameters $\etaglob$. 
...
There appears to be three dominant latent populations, which we arbitrarily label as populations 1, 2, and 3 (see \figref{stru_init_fit}). 
...
$\gpop(\etalinglobal(\alpha))$ for $\alpha = 1, \ldots, 7$. 
For this range of $\alpha$, the linearized parameters and the refitted parameters 
almost perfectly agree on values of $\gpop$ with $\tau = 0$. 
...
The posterior quantity $\gpop$ is closely related to the expected number of loci belonging to each population,
defined as
\begin{align*}
\gloci(\eta; k)
