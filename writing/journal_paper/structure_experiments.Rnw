...
<<stru_alpha_nclusters_cap>>=
  stru_alpha_nclusters_cap <- paste(
    ...
    We compute the linear approximation at $\\alpha = 3$. In the right plot, the threshold is set at $\tau = 20$ (approximately $1\\%$ of loci). ")
...
<<stru_lin_bad_example_cap>>=
  stru_lin_bad_example_cap <- paste(
    "An example where the linear approimxation poorly captured the 
    change in the admixture proportions of individual $n = 25$.")
...
<<stru_fully_lin_example_cap>>=
  stru_fully_lin_example_cap <- paste(
    "An example where a fully-linearizing the posterior statistic outperforms
