...
We consider a publicly available dataset from \citet{galbusera:2000:thrush}
that contains genotypes from  155 samples of an endangered bird species,
the Taita thrush.  
Individuals were collected from four regions in southeast Kenya 
(Chawia, Mbololo, Ngangao, Yale),
and each individual was genotyped at seven micro-satellite loci. 
The four regions were once part of a cohesive cloud forest 
that has since been fragmented by human development. 
For this endangered bird species, 
...
For example, if the measurements are all single nucleotides (A, T, C or G)
then $J_\l = 4$ for all $\l$.

We adapt STRUCTURE, a model proposed in \citet{pritchard:2000:structure, raj:2014:faststructure} for such population genetics data, 
by replacing the finite Dirichlet prior
...
Here, genetic markers take the place of
words in a document; 
in lieu of inferring ``topics," we infer latent populations.

Unlike the previous models, we now have a
stick-breaking process for each individual.
Draw sticks
\begin{align*}
...
Let $\z_{\n\l\i}$ be the assignment of observation $\x_{\n\l\i}$ to a latent population (we will use ``population" interchangably with ``cluster"
in this application).
...
The assignments are drawn from the
...
The distribution of the observed genotype $\x_{\n\l\i}\in\{1, \ldots, J_\l\}$
depends on its population assignment. Here, the components variables $\beta_\k$ 
are latent frequencies of the genotypes in population $\k$. 
Further modeling details, as well as our variational approximation,
can be found in \appref{TODO}. 

The inferred admixtures at $\alpha_0 = 3$ are shown in \figref{stru_init_fit}. 
We picked $\alpha_0 = 3$ so that \textit{a priori} 
there are $\approx 4$ populations expected under the 
GEM prior---a reasonable guess since 
we know that the data was collected from four geographic regions. 

In the posterior, there appears to be three dominant latent populations, 
which we arbitrarily label as populations 1, 2, and 3
(see \figref{stru_init_fit}). 
...
The resulting inference from our BNP model is qualitatively similar to the results reported in \citet{pritchard:2000:structure} which used the STRUCTURE model. 
Since STRUCTURE is a finite mixture model,
...
At $\alpha_0 = 3$, the BNP model agrees with \citet{pritchard:2000:structure} in that we both uncover three genetically distinct clusters,
corresponding with the Chawia, Ngangao, and Mbololo geographical regions.
...
We consider the range $\alpha = 0.5, 1, \ldots 7$. 
For this range of $\alpha$, there can be anywhere from two to seven expected populations per individual under the GEM prior (\eqref{prior_num_clusters}). 

The expected number of in-sample clusters in the data set was 
defined in \exref{insample_nclusters}, 
except in this model, the summation is over the indices for individual $n$, locus $\l$ and chromosome $i$. 
We will allow the option of setting $\tau > 0$ in order to count only the
populations that comprise a non-negligible fraction of the data set. 
...
The thresholded estimates for the number of populations is still
moderately sensitive to the value of $\alpha$. 
When refitting the variational approximation at $\alpha = 0.5, 1, \ldots, 7$,
the thresholded quantities vary between two and four
latent populations. 

The linearized variational parameters $\etalinglobal(\t)$
imperfectly captures the results observed by refitting. 
We formed the linear approximation at $\alpha_0 = 3$ and computed 
$\gclusters(\etalinglobal(\alpha))$ for $\alpha = 0.5, 1, \ldots, 7$. 
For our choices of $\alpha$, the linearized parameters and the refitted parameters 
almost perfectly agree on values of $\gclusters$ with $\tau = 0$. 
However, when $\tau = 20$,
the linearized parameters 
underestimated the true sensitivity of $\gclusters$ found by refitting. 
In particular, the linearized parameters failed to produce
the reduction to two latent populations at $\alpha = 0.5$ observed in the refits.

<<stru_alpha_nclusters_cap>>=
  stru_alpha_nclusters_cap <- paste(
    "The expected number of (thresholded) 
    ...
    $2\\%$ and $4\\%$ 
    of the total number of loci in the data set, respectively. ")
SetImageSize(aspect_ratio = 0.55 * base_aspect_ratio)
@
...
less than 
\Sexpr{ceiling(weights_alpha_init$weight[4])}
...
increases to 
$\Sexpr{sprintf('%.1f', weights_alpha_large$weight[4])}$, 
...
$\Sexpr{sprintf('%.1f', weights_alpha_small$weight[3])}$, 
...
<<stru_alpha_cluster_weights_cap>>=
  stru_alpha_cluster_weights_cap <- paste(
    "The expected number of loci per population as $\\alpha$ varies. ")
...
This suggests that some migration has occurred between the Mbololo and Ngangao regions. 
...
the multiplicatively perturbed prior 
$\p(\nu_{\n\k} \vert \t) = \p_0(\nu_{\n\k})\exp(\t\phiworstcase(\nu_{\n\k}))$ (\figref{stru_func_sens}).
Under the linearized variational parameters $\etalinglobal(\t)$, 
the admixture proportion of population 2 in the outlying Mbololo individuals is nearly halved at $\t = 1$. 
The same quantity computed after refitting the model 
confirms the sensitivity predicted by the linearized variational parameters 
...
The linearized and the refitted variational parameters again
agree on this conclusion. 
Finally, the presence of population 3 in the Chawia individuals is anticipated to be sensitive by the linearized parameters,
as this admixture proportion steadily decreases as $t\rightarrow 1$. 
...
Our sensitivity analysis suggests that the inferred migration from Mbololo to Ngangao based on the outlying Ngangao admixtures 
appears robust to our stick-breaking prior. 
On the other had, the the outlying Mbololo admixtures appears to be more sensitive to the stick-breaking prior, and conclusions about migration from Ngangao to Mbololo may be dependent on prior choices. 

\subsubsection*{Computation time}

The computational cost of linearizing the variational parameters is again favorable compared with the cost of refitting (\tabref{stru_timing}). 
...
$\glin(t)$ (\eqref{vb_g_sens}) avoids this problem. 
