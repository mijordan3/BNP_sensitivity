This section outlines two computational steps necessary for our
...
to both optimize the $\textrm{KL}$ and to
invert the Hessian for local sensitivity.

\subsection{Evaluating expectations}

Some expectations with respect to $\mathcal{Q}$ are available
analytically, by taking advantage of conditional conjugacy, for example.
...
\todo{this section really isnt about computing expectations...
we discuss a lot of variational distribution details here ... }

In the mean-field factorization for $\q$ (\eqref{vb_mf}),
the variational parameter vector $\eta$ will
partition into parameters governing $\nu$, $\beta$, and $\z$.
Let the VB parameters governing a
particular parameter vector be denoted with a subscript: for example,
$\q(\beta_\k \vert \eta) = \q(\beta_\k \vert \eta_{\beta_\k})$,
$\q(\z_\n \vert \eta) = \q(\z_\n \vert \eta_{\z_{\n}})$, and so on.
...
necessary for computing the $\mathrm{KL}$ (\eqref{vb_optimization})
will be available analytically.
...
%
To evaluate $\mathrm{KL}$, we need to compute
...
In the GMM (\exref{iris_bnp_process}),
$\beta_\k = (\mu_\k, \Lambda_\k)$, and
the likelihoods are Gaussian,
$\p(\x_\n \vert \beta_\k) = \normdist{\x_n \vert \mu_\k, \Lambda_\k^{-1}}$.
The prior $\pbetaprior(\beta_\k)$ is normal-Wishart.
Using the log densities displayed in \exref{iris_bnp_process}
and the mean-field assumption on $\q$,
observe that the variational parameters for $\beta_\k$
...
can be provided as closed-form functions of $\eta_{\beta_\k}$.
%
\end{ex}
...
Let $\rho_{\n\k}$ be the $k$-th entry of $\eta_{\z_\n}$,
in which case the multinomial expectations are
%
\begin{align*}
  p_{\n\k} := \expect{\q(\z_\n \vert \eta)}{\z_{\n\k}} =
...
$\sum_{\k=1}^\kmax p_{\n\k} = 1$, so the
optimal mean parameters cannot be
interior to $\Delta^{\kmax - 1}$.
On the other hand, $\eta_{\z_\n}$, as defined here,
is unconstrained in $\mathbb{R}^{\kmax - 1}$.

Notice from the factorization of $\logp(\x, \beta, \z, \nu)$ in \eqref{bnp_model}
and the mean-field assumption on $\q$,
the expected joint log-likelihood can be written as
\begin{align*}
  \expect{\q(\zeta\vert\eta)}{\logp(\x, \beta, \z, \nu)} =
  \sum_{\k = 1}^\kmax C_{\k} \expect{\q(\z_\n \vert \eta)}{\z_{\n\k}} + C_0,
\end{align*}
where the $C_\k$'s do not depned on $\eta_{\z_\n}$.
In other words, the expected joint log-likelihood (and thus the $\mathrm{KL}$)
...
We will numerically approximate the expectations in \eqref{stick_expectations},
which are univariate integrals.
Our choice a normal distribution on the unconstrained, logit-transformed
stick proportions $\lnu_\k$
(or equivalently, a logit-normal distribution on the constrained
stick proportions $\nu_\k$)
allows us to express the univariate expectations in \eqref{stick_expectations}
as a Gaussian integral.
Let $f(\nuk)$ be some smooth function (such as $f(\nuk) = \pstick(\nuk)$).
...
Then, we approximate the Gaussian integral
...
$\t \mapsto \pstick(\nuk \vert \t)$ we consider,
the partial derivative can be provided in either closed-form or computed with automatic differentiation.
...
of $\eta$, and we can compare how $\g$ changes with respect to $\eta$ without stochasticity.
...
\todo{not clear which part is for generic $\g$ and which is for $\gclusters$.
If we end up putting this in the appendix, we could probably just make it
about $\gclusters$}


\subsection{A global and local factorization}

All the models we consider will factorize into a set of \textit{global} latent
...
We take advantage of the fact that the latent variables
