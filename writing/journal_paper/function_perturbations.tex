...
normalizing integral is neither zero nor infinity, and we do not extrapolate to
negative alternatives $\palt(\theta)$.  (Indeed, embedding statistical problems
in non-statisical vectors spaces is a venerable technique when applying
functional analysis to statistics, see, e.g., \citet[Chapter
...
\begin{align*}
%
\int_0^1 \tilde{\p}(\theta \vert \phi) \lambda(d\theta) < \infty
\mathtxt{and}
\int_0^1 \tilde{\p}(\theta \vert \phi) \lambda(d\theta)  > 0.
%
\end{align*}
%
\begin{proof}
