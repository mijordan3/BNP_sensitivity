...
relations between $\pertset$, $\lp{p}$, and the space of valid (non-negative and
normalizable) prior densitites:
%
\begin{enumerate}
%
    \item \itemlabel{priors_are_pert}
    For any $p \in [0, \infty]$, any valid densitity $\palt \ll \pbase$
    is expressible in the form $\palt = \p(\theta \vert \phi)$ for some
    $\phi \in \pertset$.
%
    \item \itemlabel{perts_are_priors}
    If $\phi \in \pertset$, then $\p(\theta \vert \t \phi)$ is a valid
    density  for all $0 \le \t \le 1$.
%
    \item \itemlabel{perts_vs_ball_p}
    If $p \in [1, \infty)$, then $\pertset \subset \ball_{p}(p)$.
%
    \item \itemlabel{perts_vs_ball_pinf}
    If $p = \infty$, $\lp{\infty} \subset \pertset[\infty]$.
%
