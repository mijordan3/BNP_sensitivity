In this section, we show how to find worst-case functional perturbations to the stick form. In particular, we start by motivating an $\infty$-norm to measure the size of a functional perturbation $\phi$. We then show how to compute an influence function to summarize the effect of different choices of $\phi$. We prove that, for multiplicative perturbations and the $\infty$-norm, the linear approximation is uniformly good. Finally, we show that this uniformly good approximation is unique among many alternative choices of functional perturbation.
...
by changing $\alpha$ provides a counterexample; see \exref{beta_inf_norm} for more details.
...
\textcolor{red}{TODO: similar to discussion above: in next result, since we're restricting the BNP model here, can we reduce the assumption load?}
...
exchange_order_q} in \appref{diffable_nonparametric} hold. Let $g(\eta): \etadom \mapsto \mathbb{R}$ denote a
...
%
\begin{proof}
We establish and prove a similar but much more general form of the influence function for mean-field VB approximations with general choices of model and parameters in \coryref{etafun_deriv_form} in \appref{diffable_nonparametric}. We derive the special case here from that general form in \exref{infl_univariate}.
\end{proof}
...
in \secref{results}, we show how to do this by choosing $\phi$ that align with particularly high-magnitude positive or negative values of the influence function; this alignment will ensure a large positive or negative gradient and hence a large change.

%%
\noindent \textbf{Worst-case functional perturbations.}
%
With \coryref{etafun_deriv_form_stick} in hand, we can find the worst-case choice of $\phi \in \ball_\phi(\delta)$.
...
\begin{proof}
%
We establish a similar but much more general result for mean-field VB approximations with general choices of model and parameters in \coryref{etafun_worst_case}. The present result is a special case using \exref{infl_univariate}.
%
\end{proof}
...
\coryref{etafun_deriv_form}. In particular, \coryref{etafun_deriv_form}
...
to make the dependence on $\phi$ explicit. Then \coryref{etafun_deriv_form} does not imply
...
To be able to apply \coryref{etafun_worst_case} to find the worst-case perturbation $\phi$, 
...
%
Let \assuref{kl_opt_ok, exchange_order_q} hold. \textcolor{red}{Same question as above: can we get rid of appendix assumptions or have a fast summary in main text?} Then the map $\phi \mapsto
...
Consider \figref{func_dist}, which illustrates the tradeoffs between
