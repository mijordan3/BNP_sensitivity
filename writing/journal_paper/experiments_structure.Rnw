...
$\expect{\q(\pi_\n \vert \etaopt)}{\pi_\n}$ for all individuals $\n$.  Our
initial prior was a $\textrm{GEM}$ with parameter $\alpha_0 = 3$. The choice of
$\alpha_0 = 3$ corresponds to roughly four distinct populations {\em a priori}
(\eqref{prior_num_clusters}), motivated by the fact that the individuals come
from four geographic regions. Below, we will evaluate sensitivity to this prior
choice.

In the approximate posterior at $\alpha_0$, there appear to be three dominant
