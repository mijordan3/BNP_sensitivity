...
$\hessopt$ is invertible by \assuitemref{kl_opt_ok}{kl_hess}.   So we satisfy
conditions (i), (ii), and (iii) of \citet[Theorem
4.B(c)]{zeidler:2013:functional}, giving that the function $\etaopt(\phi)$
exists.
...
($C^1$ in the notation of Zeidler) in a neighborhood of $\etaopt, 0$.
...
\begin{align*}
%
\fracat{\partial \log \qtil(\theta \vert \eta)}
       {\partial \eta}{\eta}
      ={} (\theta, \theta^2)^T \mathand&
\fracat{\partial^2 \log \qtil(\theta \vert \eta)}
      {\partial \eta \partial \eta^T}{\eta}
      ={} 0_{2\times2} \Rightarrow\\
%
\norm{
    \fracat{\partial \log \qtil(\theta \vert \eta)}
           {\partial \eta}{\eta}
}_2^2 ={} \theta^2 + \theta^4 \mathand&
\norm{
    \fracat{\partial^2 \log \qtil(\theta \vert \eta)}
          {\partial \eta \partial \eta^T}{\eta}
}_2 ={} 0.
