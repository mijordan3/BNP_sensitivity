...
<<iris_alpha_sens_cap, cache=FALSE>>=
...
\begin{tabular}{|r|r|}
    ...
    Refits $\eta(\epsilon)|_{\epsilon = 1}$ for worst-case $\phi$ & 
        \Sexpr{sprintf('%1.2g', wc_refit_time)}\\ 
    \hline 
    The influence function & \Sexpr{sprintf('%1.2g', infl_time)}\\ \hline
\end{tabular}
\end{table}
