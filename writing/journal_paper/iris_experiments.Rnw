...
only from \Sexpr{sprintf("%2.1f", min_enum_clust)} to \Sexpr{sprintf("%2.1f", max_enum_clust)} (recall that the true number of iris species is three). 
On the other hand, the posterior preditive quantity is sensitive 
to changes in $\alpha$.
Over the same range of $\alpha$, $\gclusterspred$ varies from 
\Sexpr{sprintf("%2.1f", min_enum_clust_pred)} to \Sexpr{sprintf("%2.1f", max_enum_clust_pred)}. 
