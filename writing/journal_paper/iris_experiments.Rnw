...
varies only from 
\Sexpr{sprintf("%2.1f", min_enum_clust)} to
...
Forming the linear approximation at $\alpha = \alpha_0$, required \Sexpr{sprintf('%1.1g', alpha_hess_time)} seconds. 
...
$\alpha$'s took a total of \Sexpr{sprintf('%2.0f', total_alpha_refit_time)} seconds, 
...
We first consider functional perturbations of the form 
...
Each perturbation with a different choice of $\mu$ 
...
fitted with stick-distribution $p(\nu_k|\epsilon)$.
The changes in $\gclusters$
are distinct in both sign and magnitude across perturbations. 
...
In applications below, the influence function will guide our choice of functional perturbaton and explain why some perturbations result in greater sensitivity than others. 
