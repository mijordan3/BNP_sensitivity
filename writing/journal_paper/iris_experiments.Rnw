...
Without further optimization, we use the linear approximation (\eqref{global_lin_approx, global_lin_approx_all}) to quickly evaluate 
$\etalinglobal(\alpha)$ for all $\alpha = 1, \ldots, 16$,
and produce posterior quantites $g(\etalinglobal(\alpha))$. 
