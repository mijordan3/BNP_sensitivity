...
The expected in-sample number of clusters $\gclusterszero$ and 
the expected predictive number of clusters $\gclusterspredzero$ 
...
the quantity $\gclusterszero(\etalinglobal(\alpha))$
...
the predictive quantity $\gclusterspredzero(\etalinglobal(\alpha))$
varies from 
...
We present three prior perturbations, each with a different effect on the prior
and the posterior number of clusters. 
We choose these three perturbations to demonstrate the influence function .... 
...
$\Delta\gclusters(\eta(\t)) := 
\gclusters(\eta(\t)) - \gclusters(\etaopt(0))$ 
for $\t\in[0,1]$. 
Depending on the perturbation, 
$\Delta\gclusters$ can be positive, negative, or nearly zero. 
In each case, the approximation 
$\Delta\gclusters(\etalinglobal(\t))$
is able to mirror the qualitative behavior of 
$\Delta\gclusters(\etaopt(\t))$
...
On the other hand, the sign and magnitude of the change in $\gclusters$ after 
