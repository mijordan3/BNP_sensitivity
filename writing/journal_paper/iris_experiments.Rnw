...
\todo{Most of this paragraph is redundant; just say we use the model above.}
...
\todo{With $\pi_\k$ you don't need the variational expectation anymore.}
\todo{NB: I'm mostly not going to make edits other than TODOs because I don't have
the data right now and can't re-knit to make sure I'm not making typos.
Sorry...}
\begin{align*}
