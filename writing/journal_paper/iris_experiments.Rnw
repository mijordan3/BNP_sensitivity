...
The variational distribution on all latent variables except for the 
stick-breaking proportions $\nu_\k$ were taken to 
be the conditionally conjugate (\exref{TODO}). 
...
in \exref{insample_nclusters_simple}. 
...
\todo{make sure this the notation $g_{cl}$ etc. $G_0$ matches from before. }

\subsubsection*{Parametric sensitivity}

We explore how the posterior number of clusters,
either \textit{insample} or \textit{predictive}, vary with different choices
