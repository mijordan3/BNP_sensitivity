...
We explore how posterior inferences vary with different choices 
of the GEM parameter $\alpha$. 
The the expected number of clusters under the GEM prior is given by 
\begin{align}\eqlabel{prior_num_clusters}
\sum_{\n = 1}^\N \frac{\alpha}{\alpha + \n - 1},
\end{align}
where $\N$ is the number of observations CITE. 

For the iris data set, we 
consider the range $\alpha\in[0.1, 4.0]$. 
Using \eqref{prior_num_clusters}, 
the prior expected number of clusters is $\approx 1.5$ when $\alpha = 0.1$ and 
...
and without knowing the true number of iris species, it may be
unclear which $\alpha$ to choose.
...
\figref{iris_fit} shows the inferred clusters with this prior choice. 
...
the expected number of in-sample clusters $\gclusters$ and 
the expected number of predictive clusters $\gclusterspred$,
...
We again start with the variational parameters at $\alpha_0 = 2$, 
