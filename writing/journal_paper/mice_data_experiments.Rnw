...
We fit the regression model described in \exref{mice_bnp_process}
...
In this application, we expect to uncover finer structure than the iris data set, so we start with a larger $\alpha$ and set $\alpha_0 = 6$. 
