...
plot_coclustering(coclust_init) + 
  xlab('gene') + 
  ylab('gene') + 
  fontsize_theme
...
SetImageSize(aspect_ratio=base_aspect_ratio, 
             image_width = 0.8 * base_image_width)
@
<<gene_alpha_coclustering, cache=simple_cache, fig.show='hold', fig.cap=gene_alpha_coclustering_cap>>=
# source("R_scripts/mice/alpha_coclustering_matrix.R",
#        echo=knitr_debug, print.eval=TRUE)
@


<<gene_alpha_coclustering_scatter_cap>>=
gene_alpha_coclustering_scatter_cap <- paste(
    "co-clustering matrix alpha sensitivity, scatter plot")
SetImageSize(aspect_ratio=base_aspect_ratio * 0.8, 
            image_width = 0.2 * base_image_width)
@
<<gene_alpha_coclustering_scatter, cache=simple_cache, fig.show='hold', fig.cap=gene_alpha_coclustering_scatter_cap>>=
# source("R_scripts/mice/alpha_coclustering_scatter.R",
#        echo=knitr_debug, print.eval=TRUE)
@