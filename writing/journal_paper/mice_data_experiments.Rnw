...
example_genes_cap <- paste(
    "(Left) An example gene and its expression measured at 14 unique timepoints
...
alpha_pert_logphi_cap <- paste(
    "The log multiplicative perturbation $\\log \\phi_\\alpha(\\cdot)$ that 
    corresponds to decreasing the $\\alpha$ parameter by five (left) or
    increasing the $\\alpha$ parameter by five (right). ")
