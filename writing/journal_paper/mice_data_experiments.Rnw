...
have a closed form given $\q(\nu, \beta \vert \eta)$. See \secref{put_in_appendix}. 
Therefore, our model fits the global/local framework as discussed in
\secref{computing_sensitivity}; 
the global latent variables are $\gamma = (\beta, \nu)$ as before, 
but the local latent variables $(\z, \b_\n)$ now include the shifts. 
Our approximation is formed by linearizing the global parameters as before,
and we now set both $\eta_\z$ and $\eta_\b$ optimally as a function of 
the linearized global parameters.
...
$\Delta\coclusteringmatr(\etalinglobal(\alpha))$, at $\alpha = 1$ and $\alpha = 11$. 
...
took 3-4 seconds; subsequent evaluations of $\etalinglobal$ take milliseconds
(this includes setting both $\eta_\z$ and $\eta_\b$ in closed form). 
