...
Genes that cluster together may be evidence that they share a 
...
$i$-th time point.
The right plot of 
\figref{example_genes} shows the B-spline basis.
...
%
\end{align*}
Below, we will evaluate the sensitivity of the co-clustering matrix to 
both changes in the GEM parameter $\alpha$ and to functional perturbations of the stick-breaking density. 

We will use the influence function (\corref{etafun_worst_case})
to guide our choice of functional perturbation. 
To produce an easily-visualized influence function, we must choose a
...
with $\alpha_0 = 6$ as the base prior at 
...
we see that at $\alpha = \Sexpr{alpha_pert1}$, only two clusters expected
