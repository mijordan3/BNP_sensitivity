...
Below, we will use the influence function (\corref{etafun_worst_case})
to try and find a perturbation that produces large changes in the co-clustering 
matrix. 
...
we see that at $\alpha = \Sexpr{alpha_pert1}$, only two clusters are expected
