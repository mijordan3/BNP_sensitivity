...
confirms the insensitivity predicted by the linearized variational parameters.  
Beyond capturing insensitivity, the linearized variational parameters 
were also able to
...
(this includes setting both $\eta_\z$ and $\eta_\b$ in closed form after
linearizing $\eta_\gamma$). 
