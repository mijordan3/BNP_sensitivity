...
fisher:1936:iris} using a Dirichlet process prior with concentration parameter
...
concentration parameter $\alpha$ as $\alpha$ varies over some reasonable range,
...
Concretely, the VB approximation is defined by a vector of optimal
...
%
Posterior quantities of interest, such as posterior expectations, can be
...
when the approximation $\etalin(\alpha)$ is not a completely adequate substitute
for exact re-optimization, the derivative can be a useful guide for what sorts
of prior perturbations might be problematic, informing further exploration based
on re-optimizing.
