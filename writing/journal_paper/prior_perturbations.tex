...
differentiable.  To show this, it will suffice to show that the partial
deritatives of the maps $\eta \mapsto \KLgrad{\eta, \phi}$ and $\phi \mapsto
\KLgrad{\eta, \phi}$ exist and are continuous as a function of $\eta, \phi$
in the dual norm $\norm{\cdot}^*$.

By \assuref{kl_diffable}, $\eta \mapsto \KLgrad{\eta, \phiz}$ is
continuously differentiable and does not depend on $\phi$, so we need consider
only $\rho_\eta(\etanuk, \phi)$.  By \assuref{q_dominated} and the boundedness
of $\phi(\nu)$ we can apply the Lebesgue dominated convergence theorey to
differentiate under the integral (see, e.g., \citet[Theorem
1]{giordano:2018:covariances}):
%
...
\end{align}
%
Again differentiating \eqref{q_sens_is_cov} using \citet[Theorem
1]{giordano:2018:covariances} gives
%
...
We now show that $\phi \mapsto \rho_\eta(\etanuk, \phi)$ is Fr{\'e}chet
differentiable. It follows from \eqref{q_sens_is_cov} that the map $\phi \mapsto
...
\end{align*}
%
It follows from \assuref{log_q_score} that $\rho_\eta(\etanuk, \phi)$ is bounded
