...
%
The maps $\eta \mapsto \expect{\q(\nu \vert \nuk)}{\norm{\nabla \log \q(\nu
\vert \etanuk)}_2^2}$ and $\eta \mapsto \expect{\q(\nu \vert
\nuk)}{\norm{\nabla^2 \log \q(\nu \vert \etanuk)}_2}$ are continuous.
%
\end{assu}
%
Then the map $\phi \mapsto \etaopt(\phi)$ is continuously Fr{\'e}chet
differentiable as a map from $\linf$ to $\mathbb{R}^\etadim$.  Furthermore, the
derivative in the direction $\phi - \phiz$ is of the form
%
\begin{align*}
%
