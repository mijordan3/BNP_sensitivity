...
We perform a clustering analysis of Fisher's iris data set \cite{anderson:1936:iris}. Here each 
data point (with $N=150$ total points)
...
Gaussian-Wishart prior for the component parameters (detailed in \appref{app_iris})
...
matches the three iris species; see also \figref{iris_fit} in \appref{app_iris} for an illustration.
...
among $N$ data points in $[1.5,15]$ (\appref{app_beta_prior}). 
