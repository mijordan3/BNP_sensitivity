...
\end{equation}
%
\Thmref{bnp_deriv} below states that $\t \mapsto \etaopt(\t)$ is continuously
...
%
Futhermore, we show in \lemref{pert_invariance} of
\appref{diffable_nonparametric} that functional perturbations with
$\norminf{\phi} < \infty$ yield valid priors.   \Secref{influence_function}
...
\secref{influence_function} that we can compute an \emph{influence function} to
...
\secref{influence_function} that, among a class of potential functional
