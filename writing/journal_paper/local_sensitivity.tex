...
measure $\lambda$.\footnote{For the purposes of this assumption  $\q(\theta \vert
\eta)$ and $\theta$ can be taken to be some general distribution, not necessarily
corresponding to our BNP model or variational approximation.} Assume that the
map $\eta \mapsto \log \q(\theta \vert \eta)$ is twice continuously
differentiable.
...
$\eta_0 = \etaopt$, and with both $\psi(\zeta, \t) = 1$ and $\psi(\zeta, \t) =
\log \pstick(\zeta \vert \t)$, for all $\k$.
%
\end{assu}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
