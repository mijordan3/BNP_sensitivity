...
dominated convergence theorem to variational, allowing us to translate
continuity of the variational and model densitites into continuity of the
variational objective.  The details can be found in \lemref{logq_derivs,
...
$\q(\theta \vert \eta)$ are defined relative to $\lambda$ and satisfy
\assuref{dist_fun_nice} with both $\psi(\theta, \t) \equiv 1$ (no $\theta$
dependence) and with and $\psi(\theta, \t) = \log \p(\theta \vert \t)$.
%
\end{assu}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%
Let \assuref{kl_opt_ok, q_stick_regular} hold at $\eta_0 = \etaopt$ and $\t_0 =
0$.  Define\footnote{Note that if $\q(\theta \vert \eta)$ is normalized, then
...
\end{align*}
%
Then the map $\t \mapsto \etaopt(\t)$ is continuously differentiable with
derivative
%
