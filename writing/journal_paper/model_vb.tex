...
For $\z$ and $\beta$ in all models we consider, we have the option of
taking advantage of conditional conjugacy to choose distributions
$\q(\z_\n \vert \eta)$ and
$\q(\beta_\k \vert \eta)$.
...
where $\eta_{\beta_\k}$ are the normal-Wishart parameters.
...
\end{align*}
which all can be computed analytically as functions of $\eta_{\beta_\k}$.
%
\end{ex}
...
Then, in the same way as \exref{iris_var_distr},
all the necessary expectations with respect to $\nuk$ in the
$\mathrm{KL}$ can be computed analytically function of the
...
To approximate expectations, we will use numerical integration.
We choose the distribution on the logit-transformed
stick-breaking proportions
\begin{align*}
  \lnu_\k := \log(\nu_\k) - \log(\nu_\k)
\end{align*}
to be normally distributed. Letting $\s$ sigmoid function,
which provides the inverse tranformation,
\begin{align*}
    \nuk = \s(\tilde\nuk) := \frac{\exp(\lnu_\k)}{1 + \exp(\lnu_\k)}.
\end{align*}
a normal distribution on $\lnu_\k$ induces a \textit{logit-normal}
distribution on $\nuk$.
With this choice of a logit-normal distributions,
all expectations with respect to $\nu_\k$ can be expressed as a Gaussian
integral, which we approximate with GH quadrature (see \appref{gh_quadrature})
Not only do stick expectations appear in the evaluation of the $\mathrm{KL}$
they will also be necessary in computing local sensitivity (\secref{local_sensitivity}).
