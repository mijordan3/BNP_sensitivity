...
class TestInterpolation(unittest.TestCase):
    def test_spline_derivatives(self):
        ...
        grad_error = np.max(np.abs(bspline_grad_ad - bspline_grad_fin_diff))
        assert grad_error < 1e-4, \
            'error in derivative is {}'.format(grad_error)

    def test_fun_interpolation(self):
        # we'll check the interpolation at these points
        x = np.linspace(0.01, 0.99, 100)

        interp_error = np.max(np.abs(bspline_regression.eval_fun(x) - \
                    bspline_regression.eval_interp_fun(x)))

        assert interp_error < 1e-4, \
            'interpolation error: {}'.format(interp_error)
